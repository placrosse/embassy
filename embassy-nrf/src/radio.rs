...
impl<'d, T: Instance> Radio<'d, T> {
    pub fn new(
        ...
        r.frequency
            .write(|w| unsafe { w.frequency().bits((u16::from(config.frequency.clone()) - 2400) as u8) });
        ...
        // BASE0
        r.base0.write(|w| unsafe { w.base0().bits(config.base_address_0) });
        // BASE1
        r.base1.write(|w| unsafe { w.base1().bits(config.base_address_1) });
        ...
        // CRCPOLY
        r.crcpoly.write(|w| unsafe { w.bits(config.crc_poly.into()) });
        r.crcinit.write(|w| unsafe { w.bits(config.crc_init.into()) });

        // DATAWHITEIV
        r.datawhiteiv.write(|w| unsafe { w.bits(config.whitening_iv.into()) });
